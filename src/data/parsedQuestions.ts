...
export const jsTheoryQuestions: TheoryQuestion[] = [
  ...
  },
  {
    ...
    "answer": "When you copy an object or array in JavaScript, the result is either *shallow* (top-level only) or *deep* (every nested level).\n\n**Shallow copy** — copies the top-level properties only. Nested objects/arrays are still shared by reference.\n\n```js\nconst original = { a: 1, nested: { b: 2 } };\nconst shallow = { ...original };           // or Object.assign({}, original)\n\nshallow.a = 99;                            // safe\nshallow.nested.b = 99;                     // ⚠️ also mutates original.nested.b\n```\n\nCommon shallow-copy techniques: spread `{ ...obj }` / `[...arr]`, `Object.assign`, `Array.prototype.slice`, `Array.from`.\n\n**Deep copy** — recursively clones every level, so the copy is fully independent of the source.\n\n```js\nconst original = { a: 1, nested: { b: 2 } };\nconst deep = structuredClone(original);    // built-in, modern (Node 17+, all evergreen browsers)\ndeep.nested.b = 99;                        // does NOT affect original\n```\n\nOther deep-copy options:\n- `JSON.parse(JSON.stringify(obj))` — simple but loses `Date`, `Map`, `Set`, `RegExp`, functions, `undefined`, and circular references.\n- Library helpers like `lodash.cloneDeep` for full fidelity.\n\n**When to use which**\n- Shallow is fine if you only mutate the top-level fields you just copied.\n- Use deep when nested objects must remain isolated — e.g., preserving immutability for React/Redux state, or snapshotting a value before mutating."
  },
  {
    "id": "js_th_51",
    "question": "What is the `new` keyword used for?",
    "answer": "The `new` keyword is used to create an instance of a user-defined object type or a built-in object type that has a constructor function. When `new` is used, it performs four steps automatically:\n\n1. **Creates a new empty object** `{}`.\n2. **Sets the prototype** — links the new object's `__proto__` to the constructor's `prototype`.\n3. **Binds `this`** — executes the constructor function with `this` pointing to the new object.\n4. **Returns the object** — if the constructor doesn't explicitly return an object, the new object is returned automatically.\n\n```js\nfunction Person(name, age) {\n  this.name = name;\n  this.age = age;\n}\n\nPerson.prototype.greet = function() {\n  return `Hi, I'm ${this.name}`;\n};\n\nconst p = new Person('Anandu', 25);\nconsole.log(p.name);    // 'Anandu'\nconsole.log(p.greet()); // \"Hi, I'm Anandu\"\nconsole.log(p instanceof Person); // true\n```\n\n**Without `new`**: `this` inside the constructor would refer to the global object (or be `undefined` in strict mode), and no object would be returned.\n\n```js\nconst broken = Person('John', 30); // no new\nconsole.log(broken); // undefined\nconsole.log(window.name); // 'John' (leaked to global!)\n```"
  },
  {
    "id": "js_th_52",
    "question": "What is a Set in JavaScript?",
    "answer": "A `Set` is a built-in data structure that stores **unique values** of any type (primitives or object references). Duplicate values are automatically ignored.\n\n### Key Properties:\n- Values are stored in **insertion order**.\n- Lookups (`has`) are O(1) — much faster than scanning an array.\n- No key–value pairs, just values (unlike `Map`).\n\n```js\nconst set = new Set([1, 2, 3, 2, 1]);\nconsole.log(set);        // Set(3) { 1, 2, 3 } — duplicates removed\nconsole.log(set.size);   // 3\n\nset.add(4);              // { 1, 2, 3, 4 }\nset.delete(2);           // { 1, 3, 4 }\nconsole.log(set.has(3)); // true\n\n// Iterate\nfor (const val of set) {\n  console.log(val); // 1, 3, 4\n}\n\n// Common use case: remove array duplicates\nconst arr = [1, 2, 2, 3, 3, 4];\nconst unique = [...new Set(arr)]; // [1, 2, 3, 4]\n```\n\n### Set vs Array:\n| Feature | Array | Set |\n|---------|-------|-----|\n| Duplicate values | Allowed | Not allowed |\n| Order | Insertion | Insertion |\n| Lookup (`includes`/`has`) | O(n) | O(1) |\n| Index access | Yes (`arr[0]`) | No |\n\n### WeakSet:\nStores only objects, holds weak references (GC-friendly), and is not iterable. Useful for tracking objects without preventing garbage collection."
  },
  {
    "id": "js_th_53",
    "question": "Difference between `prototype` and `__proto__`",
    "answer": "`prototype` and `__proto__` are both related to JavaScript's prototype chain, but they serve different roles:\n\n- **`prototype`**: A property on **constructor functions** (and classes). It is the object that will be set as the `__proto__` of any instance created with `new`. Used to define shared methods for all instances.\n- **`__proto__`**: A property on every **object instance**. It is the actual link to the object's prototype — the object from which it inherits properties. It points to the constructor's `prototype`.\n\n```js\nfunction Animal(name) {\n  this.name = name;\n}\nAnimal.prototype.speak = function() {\n  return `${this.name} makes a sound.`;\n};\n\nconst dog = new Animal('Rex');\n\n// prototype is on the constructor\nconsole.log(Animal.prototype); // { speak: [Function] }\n\n// __proto__ is on the instance\nconsole.log(dog.__proto__ === Animal.prototype); // true\nconsole.log(dog.__proto__ === Object.getPrototypeOf(dog)); // true\n\n// Inherited method\nconsole.log(dog.speak()); // 'Rex makes a sound.'\n```\n\n**Summary**:\n- `Constructor.prototype` — blueprint used when creating instances.\n- `instance.__proto__` — the actual prototype link on each created object.\n- Prefer `Object.getPrototypeOf(obj)` over `__proto__` in production code (it's the official API)."
  },
  {
    "id": "js_th_54",
    "question": "What is `Object.create()` used for?",
    "answer": "`Object.create(proto)` creates a **new object whose prototype is set to the given object**. It gives you direct control over the prototype chain without needing a constructor function.\n\n```js\nconst animal = {\n  speak() {\n    return `${this.name} makes a sound.`;\n  }\n};\n\n// Create an object that inherits from `animal`\nconst dog = Object.create(animal);\ndog.name = 'Rex';\nconsole.log(dog.speak()); // 'Rex makes a sound.'\nconsole.log(Object.getPrototypeOf(dog) === animal); // true\n```\n\n### Key use cases:\n1. **Pure prototypal inheritance** — set up inheritance without `class` or `new`.\n2. **Creating objects with no prototype** — `Object.create(null)` creates a truly empty object (no `toString`, `hasOwnProperty`, etc.). Useful as a pure hash map.\n3. **Cloning an object's prototype chain** — used in polyfills and when manually implementing class-like patterns.\n\n```js\n// Null-prototype object (safe dictionary)\nconst map = Object.create(null);\nmap['key'] = 'value';\nconsole.log(map.hasOwnProperty); // undefined — no inherited methods\n```\n\n**vs `{}` literal**: `{}` creates an object with `Object.prototype` as its `__proto__`. `Object.create(null)` creates one with no prototype at all."
  },
  {
    "id": "js_th_55",
    "question": "What is `Object.assign()` and how does it work?",
    "answer": "`Object.assign(target, ...sources)` copies **own enumerable properties** from one or more source objects to a target object. It returns the modified target.\n\n```js\nconst target = { a: 1 };\nconst source = { b: 2, c: 3 };\n\nconst result = Object.assign(target, source);\nconsole.log(result); // { a: 1, b: 2, c: 3 }\nconsole.log(target === result); // true — target is mutated\n\n// Merging multiple sources\nconst merged = Object.assign({}, { a: 1 }, { b: 2 }, { a: 99 });\nconsole.log(merged); // { a: 99, b: 2 } — later sources overwrite earlier ones\n```\n\n### Common use cases:\n1. **Shallow clone** — `Object.assign({}, original)`\n2. **Merging defaults with user options**\n3. **Adding methods to a prototype** — `Object.assign(MyClass.prototype, mixinMethods)`\n\n### Important caveats:\n- **Shallow copy only** — nested objects are still shared by reference.\n- Copies only **own enumerable** properties (not inherited or non-enumerable ones).\n- Does **not** copy property descriptors (getters/setters become plain values).\n\n```js\nconst obj = { nested: { x: 1 } };\nconst copy = Object.assign({}, obj);\ncopy.nested.x = 99;\nconsole.log(obj.nested.x); // 99 — shared reference!\n```\n\n**Modern alternative**: Spread syntax `{ ...source }` is equivalent and more concise, but also shallow."
  },
  {
    "id": "js_th_56",
    "question": "What are Decorators in JavaScript?",
    "answer": "Decorators are a **stage 3 TC39 proposal** (and available via TypeScript with `experimentalDecorators`) that provide a way to annotate and modify classes, methods, properties, or parameters using `@expression` syntax.\n\nA decorator is a function that receives the thing being decorated and can wrap or replace it.\n\n```ts\n// Class decorator — adds a timestamp\nfunction timestamp(target: any) {\n  target.createdAt = new Date();\n}\n\n@timestamp\nclass User {\n  name: string;\n  constructor(name: string) { this.name = name; }\n}\n\nconsole.log((User as any).createdAt); // Date object\n\n// Method decorator — logs calls\nfunction log(target: any, key: string, descriptor: PropertyDescriptor) {\n  const original = descriptor.value;\n  descriptor.value = function(...args: any[]) {\n    console.log(`Calling ${key} with`, args);\n    return original.apply(this, args);\n  };\n  return descriptor;\n}\n\nclass Calculator {\n  @log\n  add(a: number, b: number) { return a + b; }\n}\n\nconst calc = new Calculator();\ncalc.add(2, 3); // logs \"Calling add with [2, 3]\", returns 5\n```\n\n### Common decorator types:\n- **Class decorators** — modify or replace the class itself.\n- **Method decorators** — wrap method behavior (logging, auth, memoization).\n- **Property decorators** — observe or transform property declarations.\n- **Parameter decorators** — annotate constructor/method parameters (used in Angular/NestJS DI).\n\n### Real-world use:\n- NestJS uses decorators heavily: `@Controller()`, `@Get()`, `@Injectable()`.\n- Angular: `@Component()`, `@NgModule()`.\n- MobX: `@observable`, `@action`."
  }
];
...
export const reactTheoryQuestions: TheoryQuestion[] = [
  ...
  },
  {
    ...
    "answer": "**React Query** (now TanStack Query) is a server-state management library for React. It treats server data as a separate concern from client (UI) state and gives you out-of-the-box caching, background refetching, deduplication, and synchronization.\n\n**Core API**\n```js\nuseQuery({ queryKey, queryFn })   // read / fetch data\nuseMutation({ mutationFn })       // create / update / delete\nqueryClient                       // central cache manager\nqueryClient.invalidateQueries()   // mark data stale → triggers refetch\nqueryClient.prefetchQuery()       // warm the cache before render\n```\n\n**Key concepts**\n- **Caching** — every `queryKey` is cached so identical reads share a single network request.\n- **Background refetching** — stale data is shown instantly, then revalidated in the background (stale-while-revalidate).\n- **Stale time** — `staleTime` defines how long cached data is considered fresh before a refetch is triggered.\n- **Retry mechanism** — failed queries are retried automatically with exponential backoff.\n- **Optimistic updates** — `onMutate` lets you patch the cache before the server responds, then rollback on error.\n- **Infinite queries** — `useInfiniteQuery` for cursor-based pagination / “load more” UX.\n- **Pagination** — `keepPreviousData: true` keeps the previous page rendered while the next page loads.\n\n**Example**\n```jsx\nimport { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';\n\nfunction Todos() {\n  const qc = useQueryClient();\n\n  const { data, isLoading } = useQuery({\n    queryKey: ['todos'],\n    queryFn: () => fetch('/api/todos').then(r => r.json()),\n    staleTime: 30_000,\n  });\n\n  const add = useMutation({\n    mutationFn: (todo) =>\n      fetch('/api/todos', { method: 'POST', body: JSON.stringify(todo) }),\n    onSuccess: () => qc.invalidateQueries({ queryKey: ['todos'] }),\n  });\n\n  if (isLoading) return <p>Loading…</p>;\n  return <ul>{data.map(t => <li key={t.id}>{t.title}</li>)}</ul>;\n}\n```\n\n**Why prefer it over rolling your own?** It removes boilerplate (loading/error flags, manual cache invalidation), prevents waterfalls via deduplication, and makes UI feel instant via stale-while-revalidate."
  },
  {
    "id": "react_th_41",
    "question": "Virtual DOM vs Shadow DOM",
    "answer": "Both contain the word *DOM* but solve completely different problems.\n\n**Virtual DOM** — a JavaScript-level concept used by React (and similar libraries).\n- A lightweight in-memory tree that mirrors the real DOM.\n- On state change, React builds a new tree, **diffs** it against the previous one (reconciliation), and patches only the changed nodes in the real DOM.\n- Goal: performance + a declarative programming model.\n\n**Shadow DOM** — a browser standard, part of Web Components.\n- A scoped DOM subtree attached to a host element via `element.attachShadow({ mode: 'open' })`.\n- Provides **encapsulation**: styles and IDs inside the shadow tree don’t leak out, and outer styles don’t bleed in.\n- Goal: build reusable custom elements (e.g., `<video>` controls, `<input type=\"date\">` pickers, custom `<my-widget>`).\n\n**Quick comparison**\n\n| Aspect | Virtual DOM | Shadow DOM |\n| --- | --- | --- |\n| Where it lives | In JS memory | In the real browser DOM |\n| Provided by | React / Vue / etc. | The browser (Web Components spec) |\n| Purpose | Efficient diffing / re-render | Style + DOM encapsulation |\n| Visible in DevTools | No | Yes (under `#shadow-root`) |\n\nThey can coexist — e.g., a React app can render a custom element that internally uses a Shadow DOM."
  },
  {
    "id": "react_th_42",
    "question": "Refresh Token Flow — how does it work?",
    "answer": "**Why two tokens?** Access tokens are short-lived (minutes) so a leak has limited blast radius; refresh tokens are long-lived (days/weeks) and used only to obtain new access tokens.\n\n**Typical flow**\n1. **Login** → server returns `accessToken` (short TTL) + `refreshToken` (long TTL).\n2. **Authenticated requests** → client sends `Authorization: Bearer <accessToken>`.\n3. **401 / token expired** → client calls `POST /auth/refresh` with the refresh token.\n4. **Server validates** the refresh token (signature + storage check), issues a new access token (and optionally rotates the refresh token).\n5. **Client retries** the original request with the new access token.\n\n**Storage recommendations**\n- Refresh token → **HttpOnly, Secure, SameSite=Strict (or Lax)** cookie. Inaccessible from JS, mitigates XSS.\n- Access token → in-memory only (or HttpOnly cookie). Avoid `localStorage`.\n\n**Concurrency** — if multiple requests fail with 401 at the same time, queue them behind a single refresh call (a “refresh promise”) and replay them once the new token arrives. Otherwise you fire N parallel refresh requests.\n\n```js\n// Axios sketch with single-flight refresh\nlet refreshing = null;\napi.interceptors.response.use(\n  (r) => r,\n  async (error) => {\n    if (error.response?.status !== 401) throw error;\n    refreshing ??= api.post('/auth/refresh').finally(() => (refreshing = null));\n    await refreshing;\n    return api.request(error.config);   // retry original\n  }\n);\n```\n\n**Hardening**\n- **Refresh token rotation** — issue a new refresh token on every refresh and invalidate the old one. Detect token reuse → revoke the whole session.\n- **Bind to device/IP/user-agent** for extra signal.\n- **Always invalidate on logout.**"
  },
  {
    "id": "react_th_43",
    "question": "Why store the access token in an HttpOnly cookie?",
    "answer": "**The threat model**\n- `localStorage` / `sessionStorage` are accessible to any JS running on the page → an XSS attacker can read tokens directly.\n- A cookie marked **HttpOnly** is *not* readable from JavaScript (`document.cookie` won’t see it), so XSS can’t exfiltrate the token.\n\n**Recommended cookie attributes**\n- `HttpOnly` — JS cannot read it.\n- `Secure` — only sent over HTTPS.\n- `SameSite=Strict` (or `Lax`) — mitigates CSRF by limiting cross-site sending.\n- `Path=/` and a sensible `Max-Age`.\n\n```http\nSet-Cookie: access_token=eyJhbGciOi...; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=900\n```\n\n**Trade-off — CSRF**\nCookies are sent automatically with same-site requests, so you trade XSS exposure for potential CSRF risk. Mitigate with:\n- `SameSite=Strict`/`Lax`,\n- a CSRF token (double-submit pattern) for state-changing requests,\n- or restrict cookie usage to a dedicated API subdomain.\n\n**Client side**\n- Use `fetch(url, { credentials: 'include' })` or `axios.defaults.withCredentials = true` so the cookie tags along.\n- Server reads it from the `Cookie` header — your code never touches the raw token.\n\n**Summary**\n- HttpOnly cookie → safer against XSS, needs CSRF protection.\n- Memory / localStorage → easier for SPAs but exposes tokens to any injected script.\nFor most production apps, HttpOnly + SameSite + CSRF token is the right default."
  },
  {
    "id": "react_th_44",
    "question": "What's new in React 19?",
    "answer": "React 19 ships several long-awaited features focused on data, forms, and concurrent rendering.\n\n**1. Actions** — async functions you pass directly to forms / transitions. React tracks pending state, errors, and optimistic updates for you.\n```jsx\nfunction NameForm() {\n  async function updateName(formData) {\n    'use server'; // or a normal client async fn\n    await api.save(formData.get('name'));\n  }\n  return <form action={updateName}><input name=\"name\" /></form>;\n}\n```\n\n**2. New hooks**\n- `useActionState(action, initial)` — returns `[state, formAction, isPending]`, ideal for forms.\n- `useFormStatus()` — read pending state of the parent `<form>` from any child (great for submit buttons).\n- `useOptimistic(state, reducer)` — show an optimistic UI value while the real action is in-flight.\n\n**3. `use()` API** — read promises and context inside render. Suspends until the promise resolves.\n```jsx\nfunction Profile({ userPromise }) {\n  const user = use(userPromise); // suspends\n  return <h1>{user.name}</h1>;\n}\n```\n\n**4. Server Components & Server Actions (stable)** — first-class support for components that run on the server with zero client JS, plus form-friendly server actions.\n\n**5. `ref` as a prop** — function components can now accept `ref` directly without `forwardRef`.\n```jsx\nfunction Input({ ref, ...props }) {\n  return <input ref={ref} {...props} />;\n}\n```\n\n**6. Document metadata** — `<title>`, `<meta>`, `<link>` rendered inside components are hoisted to `<head>` automatically; no more `react-helmet`.\n\n**7. Stylesheets & async scripts** — built-in support with deduping and priority hints.\n\n**8. Better hydration errors** — clearer diffs that point to the offending element.\n\n**9. Context as provider** — `<MyContext value={...}>` instead of `<MyContext.Provider value={...}>`.\n\n**10. Improved `useDeferredValue`** — accepts an initial value to avoid showing a placeholder on first render."
  },
  {
    "id": "react_th_45",
    "question": "Controlled vs Uncontrolled Components",
    "answer": "The distinction is *who owns the form input’s value*.\n\n**Controlled** — React state owns the value. The input reads from state and writes via `onChange`.\n```jsx\nfunction Controlled() {\n  const [name, setName] = useState('');\n  return (\n    <input value={name} onChange={(e) => setName(e.target.value)} />\n  );\n}\n```\n- ✅ Single source of truth, easy validation, easy conditional disabling, easy to reset programmatically.\n- ⚠️ Re-renders on every keystroke (rarely a problem unless you have 100+ inputs).\n\n**Uncontrolled** — the DOM owns the value. You read it via a ref when needed, usually on submit.\n```jsx\nfunction Uncontrolled() {\n  const ref = useRef(null);\n  const submit = () => console.log(ref.current.value);\n  return (\n    <>\n      <input defaultValue=\"\" ref={ref} />\n      <button onClick={submit}>Submit</button>\n    </>\n  );\n}\n```\n- ✅ Less code, no re-renders per keystroke, plays nicely with non-React libraries / file inputs.\n- ⚠️ Validation and live UX are harder; state isn’t reactive.\n\n**Rules of thumb**\n- Default to **controlled** for app forms.\n- Reach for **uncontrolled** for `<input type=\"file\">`, integrating third-party widgets, or extreme perf cases.\n- Form libraries like React Hook Form lean uncontrolled by design, giving you the perf benefits while still handling validation."
  },
  {
    "id": "react_th_46",
    "question": "What is middleware (in Redux / Redux Toolkit)?",
    "answer": "**Middleware** sits between `dispatch(action)` and the reducer, letting you intercept, transform, log, or postpone actions. It’s how Redux supports async work without modifying reducers.\n\n**Mental model**\n```\ndispatch(action) → [ middleware 1 → middleware 2 → … ] → reducer → new state\n```\n\nEach middleware has the signature `({ getState, dispatch }) => next => action => { ... }`.\n\n**Built-in / common middleware**\n- **redux-thunk** (default in Redux Toolkit) — lets action creators return functions that receive `dispatch` and `getState`.\n  ```js\n  const fetchUser = (id) => async (dispatch) => {\n    dispatch({ type: 'user/loading' });\n    const data = await api.getUser(id);\n    dispatch({ type: 'user/loaded', payload: data });\n  };\n  ```\n- **redux-saga** — generator-based, models complex async flows (cancellation, debounce, race).\n- **redux-logger** — logs every action and the resulting state, great in dev.\n- **RTK Query / `createAsyncThunk`** — the modern way; built on top of thunk middleware.\n\n**Custom middleware example** (a tiny logger):\n```js\nconst logger = (store) => (next) => (action) => {\n  console.log('dispatching', action);\n  const result = next(action);\n  console.log('next state', store.getState());\n  return result;\n};\n```\n\n**Wiring it up with Redux Toolkit**\n```js\nimport { configureStore } from '@reduxjs/toolkit';\nimport logger from './logger';\n\nexport const store = configureStore({\n  reducer: rootReducer,\n  middleware: (getDefault) => getDefault().concat(logger),\n});\n```\n\n**Why care?**\n- Keeps reducers pure (synchronous, deterministic).\n- Centralizes cross-cutting concerns: API calls, auth, analytics, retry/backoff, caching.\n- Composes — chain as many as you need."
  }
